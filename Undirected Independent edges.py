...
# plt.plot(
#     n_values, 10 ** (regression_slope * log_n_values + regression_intercept),
#     color='red', label=f'Regression: slope={regression_slope:.2f}'
# )
plt.plot(n_values, Average_Consensus_times, 'o')
plt.plot(n_values, theoretical_values * reg_slope, '-', color="green",
         label=r'$N^{\frac{2}{3}}$')
...
# plt.title(f'Alpha = {alpha}, Theoretical slope = {2 * (nu - 2) / (nu - 1)}, Regression slope = {regression_slope}')
plt.title(f'ER p = {l}/n - Iterations VM: {iteration_VM}, Graph: {iteration_Graph}')
plt.show()
